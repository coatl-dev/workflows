...
jobs:
  pip-compile-upgrade:
    uses: coatl-dev/workflows/.github/workflows/pip-compile-upgrade.yml@v3.0.1
    with:
...
jobs:
  pre-commit-autoupdate:
    uses: coatl-dev/workflows/.github/workflows/pre-commit-autoupdate.yml@v3.0.1
    with:
      skip-repos: 'flake8'
    secrets:
      gh-token: ${{ secrets.GH_TOKEN }}
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/pre-commit.yml@v3.0.1
    with:
      skip-hooks: 'pylint'
```
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/pylint.yml@v3.0.1
```
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/pypi-upload.yml@v3.0.1
    with:
      python-version: '3.11'
    secrets:
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/tox-docker.yml@v3.0.1
```
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/tox-envs.yml@v3.0.1
    with:
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/tox-gh.yml@v3.0.1
    with:
      python-versions: '["3.7", "3.8", "3.9", "3.10", "3.11", "3.12"]'
```
...
jobs:
  main:
    uses: coatl-dev/workflows/.github/workflows/tox.yml@v3.0.1
```
